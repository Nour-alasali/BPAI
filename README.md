...
considerable time; the graph-generation cell writes `recipes.ttl` in the repository
...
checks a small RDF/SPARQL query, and checks the Dash startup API. It does not
execute the complete dataset analysis or access Wikidata.

## Reproducibility status

This environment establishes a new baseline. The original notebook records
Python 3.11.5 but does not record its library or model versions, so the new setup
does not establish equivalence with the original research results.

Remaining work includes unique recipe identifiers (names currently merge records),
cached geographic data, deterministic processing and output checks, and automated
fresh-install verification in GitHub Actions. The project is not yet fully
